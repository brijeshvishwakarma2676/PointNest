...
/**
 * Reusable AddPurchaseForm component that can be rendered inline or inside a sliding Drawer.
 */
export const AddPurchaseForm = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (pointsRequired < 0) {
          toast.error("Points to redeem cannot be negative.");
          setSubmitting(false);
          return;
        }
  ...
  const executeTransaction = async () => {
    ...
    try {
      ...
      if (response.success || response.id) {
        ...
        setPurchase({ phone: initialPhone, amount: "" });
        setPointsToRedeem("");
        setAppliedCoupon(null);
        setCouponCode("");
  ...
  const handleCreateCustomer = async (e) => {
    ...
    try {
      ...
      if (response.success || response.id) {
        ...
        // Refresh customer details if phone is correct
        const detailsRes = await customersApi.getCustomerDetails({ phone: purchase.phone });
  ...
  const formContent = (
    <form onSubmit={handleSubmit} className={isDrawer ? "p-0 space-y-6" : "p-8 md:p-12"}>
      <div className={isDrawer ? "space-y-6" : "grid grid-cols-1 md:grid-cols-2 gap-10"}>
          ...
          <div>
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-14 pr-6 py-5 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-gray-900 outline-none transition-all text-gray-900 font-bold text-lg placeholder-gray-300 disabled:opacity-60"
      ...
      <div className="mt-12 flex flex-col sm:flex-row items-center gap-6">
        <button
          ...
          className="flex-1 w-full flex items-center justify-center gap-3 bg-[#0A0A0B] hover:bg-gray-800 text-white px-10 py-5 rounded-2xl shadow-xl shadow-gray-200 transition-all font-bold text-base active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed group"
        >
          {submitting ? (
        ...
        {isDrawer && onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="w-full sm:w-auto px-8 py-5 border border-gray-200 rounded-2xl font-bold text-gray-500 hover:bg-gray-50 transition-colors text-sm"
          >
            Cancel
          </button>
        )}
  ...
  return (
    <>
      ...
      {isConfirming && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 bg-[#0A0A0B]/80 backdrop-blur-md animate-in fade-in duration-500">
      ...
      {notFoundModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 bg-[#0A0A0B]/60 backdrop-blur-sm animate-in fade-in duration-300">
...
const AddPurchase = () => {
  ...
  return (
    <div className="p-6 md:p-10 max-w-4xl mx-auto min-h-[90vh] flex flex-col justify-center animate-in fade-in duration-700">
      ...
      <AddPurchaseForm initialPhone={initialPhone} />
    </div>
  );
};
